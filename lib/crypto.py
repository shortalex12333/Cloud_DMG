...
2. First run: Agent sends yacht_id + yacht_id_hash to /register
3. Cloud validates hash, sends activation email to buyer
4. Buyer clicks link, cloud generates shared_secret
5. Agent polls /check-activation, receives shared_secret ONE TIME
...
def generate_installation_manifest(yacht_id: str) -> Dict[str, Any]:
    ...
    return {
        ...
        'api_endpoint': 'https://qvzmkaamzaqxpzbewjxe.supabase.co',  # Supabase for check-activation
        'n8n_endpoint': 'https://api.celeste7.ai/webhook',  # n8n for registration
    }
